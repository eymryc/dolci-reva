...
import { usePublicLounge, useBookLounge } from '@/presentation/hooks/useLounges';
...
export default function LoungeDetailScreen() {
  ...
  const loungeId = Number(id);
  const { data: lounge, isLoading } = usePublicLounge(loungeId);
  const bookMutation = useBookLounge(loungeId);
  ...
  const galleryUrls = lounge.gallery_images?.length
    ? lounge.gallery_images.map((g) => g.large_url ?? g.medium_url ?? g.url)
    ...
    <View className="flex-1 bg-white">
      <ScrollView contentContainerClassName="pb-8">
        ...
        <View className="gap-4 p-6">
          <View>
            <Text className="font-rajdhani-bold text-2xl text-theme-secondary">{lounge.name}</Text>
          ...
          {lounge.description && (
            <Text className="text-sm leading-5 text-gray-600">{lounge.description}</Text>
          )}

          <View className="flex-row flex-wrap gap-2">
            <Badge variant="secondary">{lounge.venue_type === 'BAR' ? 'Bar' : 'Lounge'}</Badge>
            {!!lounge.age_restriction && <Badge variant="secondary">{`${lounge.age_restriction}+`}</Badge>}
            {lounge.outdoor_seating && <Badge variant="secondary">Terrasse</Badge>}
          </View>
