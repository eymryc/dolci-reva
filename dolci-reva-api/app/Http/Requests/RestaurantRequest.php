...
class RestaurantRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Legacy alias from older front payloads
        if (!$this->has('feature_option_ids') && $this->has('amenities')) {
            $amenities = $this->input('amenities');
            if (is_string($amenities)) {
                $amenities = array_values(array_filter(array_map('intval', explode(',', $amenities))));
            }
            $this->merge(['feature_option_ids' => is_array($amenities) ? $amenities : []]);
        }
