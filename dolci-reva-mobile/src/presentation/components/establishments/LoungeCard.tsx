...
import { MapPin, Wine } from 'lucide-react-native';
import { Text } from '@/presentation/components/ui/Text';
import { colors } from '@/core/theme/colors';
import type { Lounge } from '@/domain/entities/lounge';
...
export function LoungeCard({ lounge }: { lounge: Lounge }) {
  ...
  return (
    <Pressable
      onPress={() => router.push(`/(tabs)/lounges/${lounge.id}`)}
    ...
    >
      <View className="relative h-44 w-full bg-gray-100">
        <Image
          source={{ uri: lounge.main_image_url ?? undefined }}
          style={{ width: '100%', height: '100%' }}
          contentFit="cover"
          transition={200}
        />
        <View className="absolute left-3 top-3 flex-row items-center gap-1 rounded-full bg-theme-primary px-3 py-1.5">
          <Wine size={12} color="white" />
          <Text className="text-xs font-rajdhani-semibold text-white">
            {lounge.venue_type === 'BAR' ? 'Bar' : 'Lounge'}
          </Text>
        </View>
      </View>
      ...
      <View className="gap-1.5 p-4">
        ...
        <View className="flex-row items-center gap-1">
          ...
          <Text className="text-sm text-gray-500">{lounge.city}</Text>
        </View>
      </View>
    </Pressable>
  );
