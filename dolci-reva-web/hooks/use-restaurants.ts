...
// ========== Restaurants publics (front-office, sans auth) ==========

export function usePublicRestaurants(filters?: { search?: string; city?: string }) {
  return useQuery({
    queryKey: ["public", "restaurants", filters],
    queryFn: async () => {
      const params: Record<string, string> = {};
      if (filters?.search) params.search = filters.search.trim();
      if (filters?.city) params.city = filters.city.trim();
      const response = await api.get("/public/restaurants", { params });
