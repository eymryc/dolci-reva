...
import { VerificationRequiredAlert } from "@/components/admin/shared/VerificationRequiredAlert";
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
...
interface NavItem {
  ...
  href: string;
  permission?: () => boolean;
  ownerKey?: OwnerNavKey;
}

const adminNavItems: NavItem[] = [
  { name: "Dashboard", icon: LayoutDashboard, href: "/admin/dashboard" },
  { name: "Profil", icon: User, href: "/admin/profile", permission: () => true },
  { name: "Résidences", icon: Home, href: "/admin/residences", permission: () => true },
  { name: "Hebergements", icon: Home, href: "/admin/hebergements", permission: () => true },
  { name: "Hôtels", icon: Building2, href: "/admin/hotels", permission: () => true },
  { name: "Restaurants", icon: UtensilsCrossed, href: "/admin/restaurants", permission: () => true },
  { name: "Lounges", icon: Coffee, href: "/admin/lounges", permission: () => true },
  { name: "Bars", icon: Wine, href: "/admin/bars", permission: () => true },
  { name: "Night-Clubs", icon: Music2, href: "/admin/night-clubs", permission: () => true },
];

/** Nav allégée « espace hôte » — URLs /proprietaire */
const ownerNavItems: NavItem[] = [
  {
    name: "Accueil",
    icon: LayoutDashboard,
    href: hostPath("/dashboard"),
    permission: () => true,
    ownerKey: "dashboard",
  },
  {
    ...
    icon: Store,
    href: hostPath("/establishments"),
    permission: () => true,
    ownerKey: "establishments",
  },
  {
    name: "Réservations",
    icon: Calendar,
    href: hostPath("/bookings"),
    permission: () => true,
    ownerKey: "bookings",
  },
  {
...
const getPageInfo = (pathname: string) => {
  const routes: Record<string, { title: string; breadcrumbs: Array<{ label: string; href?: string }> }> = {
    ...
    "/admin/establishments": {
      ...
      breadcrumbs: [{ label: "Mes établissements" }],
    },
    "/admin/bookings": {
      title: "Réservations",
    ...
    "/admin/lounges": {
      title: "Lounges",
      breadcrumbs: [{ label: "Lounges" }],
    },
    "/admin/lounges/new": {
      title: "Nouveau lounge",
      breadcrumbs: [{ label: "Lounges", href: "/admin/lounges" }, { label: "Nouveau lounge" }],
    },
    "/admin/night-clubs": {
      title: "Night-Clubs",
      breadcrumbs: [{ label: "Night-Clubs" }],
    },
    "/admin/night-clubs/new": {
      title: "Nouveau night-club",
      breadcrumbs: [{ label: "Night-Clubs", href: "/admin/night-clubs" }, { label: "Nouveau night-club" }],
    },
    "/admin/bars": {
      title: "Bars",
      breadcrumbs: [{ label: "Bars" }],
    },
    "/admin/bars/new": {
      title: "Nouveau bar",
      breadcrumbs: [{ label: "Bars", href: "/admin/bars" }, { label: "Nouveau bar" }],
    },
    "/admin/settings": {
      title: "Paramètres",
    ...
    "/admin/operations": {
      title: "Suivi monétique",
      breadcrumbs: [
        { label: "Finance", href: "/admin/operations" },
        { label: "Suivi" },
      ],
    },
  };
  ...
  if (
    pathname.startsWith("/admin/residences/") &&
  ...
  if (pathname.startsWith("/admin/lounges/") && pathname.includes("/edit")) {
    return {
      title: "Modifier le lounge",
      breadcrumbs: [{ label: "Lounges", href: "/admin/lounges" }, { label: "Modifier" }],
    };
  }

  if (pathname.startsWith("/admin/night-clubs/") && pathname.includes("/edit")) {
    return {
      title: "Modifier le night-club",
      breadcrumbs: [{ label: "Night-Clubs", href: "/admin/night-clubs" }, { label: "Modifier" }],
    };
  }

  if (pathname.startsWith("/admin/bars/") && pathname.includes("/edit")) {
    return {
      title: "Modifier le bar",
      breadcrumbs: [{ label: "Bars", href: "/admin/bars" }, { label: "Modifier" }],
    };
  }
...
export default function AdminLayout({
  ...
  const isHostView = ownerUser && !isAnyAdmin();
  const { data: ownerVerification } = useVerificationStatus(ownerUser);
  const { data: platformWallet } = usePlatformWallet(isAnyAdmin() && !isHostView);

  // Redirection URL : hôte sous /proprietaire, admin hors /proprietaire
  useEffect(() => {
    if (loading || !user) return;
    if (isHostView && pathname.startsWith("/admin")) {
      router.replace(swapBackofficeBase(pathname, "host"));
      return;
    }
    if (isAnyAdmin() && pathname.startsWith("/proprietaire")) {
      router.replace(swapBackofficeBase(pathname, "admin"));
    }
  }, [loading, user, isHostView, pathname, router, isAnyAdmin]);

  // Statut : endpoint dédié en priorité, sinon profil auth
  const verificationStatus = (
    ownerVerification?.verification?.verification_status ??
    user?.verification_status
  )
    ?.trim()
    .toUpperCase();
  const isOwnerVerified = verificationStatus === "APPROVED";
  // Afficher pour tout propriétaire non approuvé (y compris statut absent)
  const isOwnerNotVerified = ownerUser && !isOwnerVerified;
  
  // Masquer l'alerte une fois le compte approuvé
  useEffect(() => {
    if (isOwnerVerified) {
  ...
  // Propriétaire : nav hôte allégée ; admin : console complète
  const baseNavItems = isHostView ? ownerNavItems : adminNavItems;
  ...
  // Fonction pour déconnecter l'utilisateur (redirect géré uniquement par AuthContext.logout)
  const handleLogout = () => {
    logout();
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50">
      ...
      >
        ...
        <nav className="relative z-10 flex-1 px-2 sm:px-4 py-3 sm:py-6 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const isActive = isHostView
              ? item.ownerKey
                ? isOwnerNavActive(pathname, item.ownerKey)
                : false
              : pathname === item.href ||
            ...
            return (
              <Link
                ...
                className={`group relative flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-2 sm:py-3 transition-all duration-200 ${
                  isHostView
                    ? isActive
                      ? "rounded-none border-l-[3px] border-[#f08400] bg-[#f08400] text-white shadow-[0_10px_24px_-12px_rgba(240,132,0,0.85)]"
                      : "rounded-none border-l-[3px] border-transparent text-slate-600 hover:bg-[#fff8f0] hover:text-slate-900"
                    : isActive
                      ? "rounded-none bg-[#f08400] text-white shadow-[0_10px_24px_-12px_rgba(240,132,0,0.85)]"
                      : "rounded-none text-gray-700 hover:bg-[#fff8f0] hover:text-slate-900"
                }`}
              >
                <Icon
                ...
                {isActive && (
                  <div className="absolute right-1 sm:right-2 h-1.5 w-1.5 bg-white sm:h-2 sm:w-2"></div>
                )}
              </Link>
            );
          })}
          ...
          {(isSidebarOpen || isDesktop) && (
            <div className="pt-4 sm:pt-8 mt-4 sm:mt-8 border-t border-gray-200/50">
              {isAnyAdmin() && (
                ...
                >
                  ...
                  {(pathname === "/admin/settings" || pathname.startsWith("/admin/settings/")) && (
                    <div className="absolute right-1 sm:right-2 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-white"></div>
                  )}
                </Link>
              )}
              {canManageUsers() && (
              ...
              {/* Finance — console monétique admin (hôtes : Portefeuille dans la nav) */}
              {isAnyAdmin() && (
                <Link
                  ...
                  className={`group relative flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl transition-all duration-200 ${pathname === "/admin/operations" || pathname.startsWith("/admin/operations/") || pathname === "/admin/transactions"
                      ? "rounded-none bg-[#f08400] text-white shadow-[0_10px_24px_-12px_rgba(240,132,0,0.85)]"
                      : "text-gray-700 hover:bg-gradient-to-r hover:from-gray-100 hover:to-gray-50 hover:shadow-md"
                  }`}
                >
                  <Landmark className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ${pathname === "/admin/operations" || pathname.startsWith("/admin/operations/") || pathname === "/admin/transactions" ? "scale-110" : "group-hover:scale-110"}`} />
                  <span className={`text-xs sm:text-sm font-medium transition-all ${pathname === "/admin/operations" || pathname.startsWith("/admin/operations/") || pathname === "/admin/transactions" ? 'text-white' : 'text-gray-700'}`}>Finance</span>
                  {(pathname === "/admin/operations" || pathname.startsWith("/admin/operations/") || pathname === "/admin/transactions") && (
                    <div className="absolute right-1 sm:right-2 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-white"></div>
                  )}
                </Link>
              )}
      ...
      <div className="flex-1 flex min-w-0 flex-col overflow-hidden">
        ...
        <header className="h-auto min-h-[60px] sm:min-h-[70px] lg:h-20 bg-white/80 backdrop-blur-sm border-b border-gray-200/50 px-1.5 sm:px-2 lg:px-6 py-1 sm:py-1.5 lg:py-0 flex flex-row items-center justify-between gap-1 sm:gap-1.5 lg:gap-3 shadow-sm overflow-x-auto">
          <div className="flex items-center gap-1 sm:gap-2 lg:gap-4 shrink-0 min-w-0 max-w-[28%] xl:max-w-[32%]">
            <button 
            ...
            {/* Page Title and Breadcrumb — masqué pour les hôtes (évite le double titre avec HostPageHeader) */}
            {!(isHostView) && (
              <div className="hidden lg:flex flex-col gap-2 min-w-0 pl-2">
          ...
          {/* Alerte vérification — espace central navbar */}
          <div className="flex min-w-0 flex-1 items-center justify-center px-1 sm:px-2 xl:px-4">
            {isOwnerNotVerified && showVerificationAlert && (
              <VerificationRequiredAlert
                href={`${isHostView ? "/proprietaire" : "/admin"}/profile?tab=verification`}
          ...
          <div className="flex items-stretch gap-0.5 sm:gap-1 lg:gap-3 flex-shrink-0">
            {/* Wallet — hôte : escrow ; admin : commissions plateforme */}
            {user && isHostView && (
              <div className="flex items-center gap-0.5 sm:gap-1 xl:gap-3 2xl:gap-4 px-1 sm:px-1.5 xl:px-3 2xl:px-4 h-7 sm:h-auto sm:py-1 xl:py-1.5 2xl:py-2 bg-gradient-to-r from-[#f08400]/10 to-[#f08400]/5 rounded-md sm:rounded-lg border border-[#f08400]/20 shadow-sm">
                <div
                  className="flex items-center gap-0.5 sm:gap-1 lg:gap-2 xl:gap-3"
                  title="Argent déjà versé sur votre compte (après le check-in du client). Vous pouvez le retirer."
                >
                  ...
                  <div className="flex flex-col">
                    <span className="hidden sm:inline text-[8px] lg:text-[10px] xl:text-xs text-gray-500 font-medium leading-tight">Sur mon compte</span>
                ...
                <div
                  className="flex items-center gap-0.5 sm:gap-1 lg:gap-2 xl:gap-3"
                  title="Le client a déjà payé. L'argent est sécurisé et arrivera sur votre compte au check-in."
                >
                  ...
                  <div className="flex flex-col">
                    <span className="hidden sm:inline text-[8px] lg:text-[10px] xl:text-xs text-gray-500 font-medium leading-tight">Payé (bloqué)</span>
                ...
                <div
                  className="flex items-center gap-0.5 sm:gap-1 lg:gap-2 xl:gap-3"
                  title="Réservations créées mais pas encore payées par le client."
                >
                  <div className="p-0.5 sm:p-1 lg:p-1.5 xl:p-2 bg-amber-50 rounded sm:rounded-lg">
                    <Receipt className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-3.5 lg:h-3.5 xl:w-4 xl:h-4 text-amber-600" />
                  </div>
                  <div className="flex flex-col">
                    <span className="hidden sm:inline text-[8px] lg:text-[10px] xl:text-xs text-gray-500 font-medium leading-tight">Pas encore payé</span>
                    <span className="text-[9px] sm:text-[10px] lg:text-xs xl:text-sm font-bold text-amber-800 leading-tight whitespace-nowrap">
                      {new Intl.NumberFormat('fr-FR', {
                        style: 'currency',
                        currency: 'XOF',
                        maximumFractionDigits: 0,
            ...
            {user && isAnyAdmin() && !isHostView && (
              <Link
                href="/admin/operations"
                className="flex items-center gap-0.5 sm:gap-1 xl:gap-3 2xl:gap-4 px-1 sm:px-1.5 xl:px-3 2xl:px-4 h-7 sm:h-auto sm:py-1 xl:py-1.5 2xl:py-2 bg-gradient-to-r from-[#f08400]/10 to-[#f08400]/5 rounded-md sm:rounded-lg border border-[#f08400]/20 shadow-sm hover:border-[#f08400]/40 transition-colors"
                title="Commissions plateforme et file de retraits"
              >
                <div className="flex items-center gap-0.5 sm:gap-1 lg:gap-2 xl:gap-3">
                  <div className="p-0.5 sm:p-1 lg:p-1.5 xl:p-2 bg-[#f08400]/10 rounded sm:rounded-lg">
                    <Landmark className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-3.5 lg:h-3.5 xl:w-4 xl:h-4 text-[#f08400]" />
                  </div>
                  <div className="flex flex-col">
                    <span className="hidden sm:inline text-[8px] lg:text-[10px] xl:text-xs text-gray-500 font-medium leading-tight">
                      Commissions
                    </span>
                    <span className="text-[9px] sm:text-[10px] lg:text-xs xl:text-sm font-bold text-gray-900 leading-tight whitespace-nowrap">
                      {new Intl.NumberFormat("fr-FR", {
                        ...
                        currency: "XOF",
                        maximumFractionDigits: 0,
                      }).format(Number(platformWallet?.balance) || 0)}
                    </span>
                  </div>
                </div>

                <div className="h-3 sm:h-4 lg:h-6 xl:h-8 w-px bg-gray-300" />

                <div className="flex items-center gap-0.5 sm:gap-1 lg:gap-2 xl:gap-3">
                  <div className="p-0.5 sm:p-1 lg:p-1.5 xl:p-2 bg-amber-50 rounded sm:rounded-lg">
                    <Clock3 className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-3.5 lg:h-3.5 xl:w-4 xl:h-4 text-amber-600" />
                  </div>
                  <div className="flex flex-col">
                    <span className="hidden sm:inline text-[8px] lg:text-[10px] xl:text-xs text-gray-500 font-medium leading-tight">
                      Retraits à traiter
                    ...
                    <span className="text-[9px] sm:text-[10px] lg:text-xs xl:text-sm font-bold text-amber-800 leading-tight whitespace-nowrap">
                      {platformWallet?.pending_withdrawals ?? 0}
                    </span>
                  </div>
                </div>
              </Link>
