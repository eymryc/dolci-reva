...
function Carousel({ images }: { images: string[] }) {
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-3">
      {/* Image principale - 50% */}
      <div className="w-full lg:w-1/2 relative aspect-[16/10] rounded-none overflow-hidden bg-gray-100 shadow-md group">
        <Image
        ...
        <div className="grid grid-cols-3 gap-2">
          {visibleThumbnails.map((image, idx) => (
            <button
              key={idx}
              onClick={() => goTo(idx)}
              className={`relative aspect-square rounded-none overflow-hidden transition-all duration-300 ${
                idx === current 
          ...
          {hasMoreThanMax && (
            <button
              ...
              className="relative aspect-square rounded-none bg-gradient-to-br from-theme-primary/10 to-theme-accent/10 flex items-center justify-center border-2 border-theme-primary/30 hover:border-theme-primary transition-all duration-300 cursor-pointer"
...
export default function HebergementDetailPage() {
  ...
  // Format price (API may still send decimal strings — always coerce to int)
  ...
  const formatPrice = (price: string | number | null | undefined) => {
    return toMoney(price).toLocaleString("fr-FR", {
      minimumFractionDigits: 0,
      maximumFractionDigits: 0,
    });
  };

  const rent = toMoney(dwelling?.rent);
  const advance = toMoney(
    dwelling?.rent_advance_amount ||
      (dwelling?.rent_advance_amount_number
        ? rent * toMoney(dwelling.rent_advance_amount_number)
        : 0)
  );
  const deposit = toMoney(
    dwelling?.security_deposit_amount ||
      (dwelling?.security_deposit_month_number
        ? rent * toMoney(dwelling.security_deposit_month_number)
        : 0)
  );
  const agencyFees = toMoney(
    dwelling?.agency_fees ||
      (dwelling?.agency_fees_month_number
        ? rent * toMoney(dwelling.agency_fees_month_number)
        : 0)
  );
  const initialTotal = advance + deposit + agencyFees;
        ...
        <div className="flex flex-col lg:flex-row gap-6">
              ...
              <Card className="p-4 lg:p-6 shadow-md border border-gray-100 bg-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {dwelling.rent_advance_amount_number && dwelling.rent_advance_amount_number > 0 && (
                    <div className="flex items-center gap-3 p-4 rounded-none bg-blue-50 border border-blue-200">
                      <div className="w-10 h-10 bg-blue-100 rounded-none flex items-center justify-center">
                  ...
                  {dwelling.security_deposit_month_number && dwelling.security_deposit_month_number > 0 && (
                    <div className="flex items-center gap-3 p-4 rounded-none bg-amber-50 border border-amber-200">
                      <div className="w-10 h-10 bg-amber-100 rounded-none flex items-center justify-center">
                  ...
                  {dwelling.agency_fees_month_number && dwelling.agency_fees_month_number > 0 && (
                    <div className="flex items-center gap-3 p-4 rounded-none bg-purple-50 border border-purple-200">
                      <div className="w-10 h-10 bg-purple-100 rounded-none flex items-center justify-center">
                  ...
                  {dwelling.visite_price && dwelling.visite_price > 0 && (
                    <div className="flex items-center gap-3 p-4 rounded-none bg-green-50 border border-green-200">
          ...
          <aside className="w-full lg:w-96">
            <Card className="p-6 lg:p-8 sticky top-24 shadow-2xl border-0 rounded-3xl bg-gradient-to-br from-white to-gray-50/50 backdrop-blur-sm">
              ...
              <div className="space-y-3 mb-8 p-5 bg-[#faf8f5] rounded-none border border-gray-100">
                ...
                <div className="border-t-2 border-gray-300 pt-4 mt-2">
                  <div className="flex justify-between items-center gap-3">
                    ...
                    <span className="text-right text-2xl font-extrabold text-theme-primary whitespace-nowrap">
                      {formatPrice(initialTotal)} FCFA
                    </span>
                  </div>
                </div>
              </div>
              ...
              <Button 
                ...
                className="w-full bg-[#f08400] hover:bg-[#d97400] text-white py-5 rounded-none font-bold text-lg shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-0.5"
              >
                Demander une visite
              </Button>
