...
export function usePublicResidences(filters?: PublicResidencesFilters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.min_guests != null && filters.min_guests !== '') params.min_guests = String(filters.min_guests);
      if (filters?.min_bedrooms != null && filters.min_bedrooms !== '') params.min_bedrooms = String(filters.min_bedrooms);
    ...
    mutationFn: async ({
      ...
      Object.entries(data).forEach(([key, value]) => {
        if (key === "feature_option_ids") {
          // Handle feature_option_ids array separately
          if (Array.isArray(value) && value.length > 0) {
            value.forEach((id) => {
              formData.append("feature_option_ids[]", id.toString());
            });
          }
...
export function useUpdateResidence() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await api.post<ApiResponse<Residence>>(`/residences/${id}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
