...
export function LoungeForm({
  ...
  // Le catalogue de caractéristiques dépend du type de lieu ; NIGHT_CLUB en
  // priorité si sélectionné (zones/night-clubs ont des équipements dédiés),
  // sinon LOUNGE (couvre aussi les bars, cf. venue_type=BAR).
  const venueTypeForFeatures = watch("venue_type")?.includes("NIGHT_CLUB") ? "NIGHT_CLUB" : "LOUNGE";
        ...
        <div className="space-y-3 p-2 sm:p-3">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className={`${errors.age_restriction ? "border-red-500" : ""} h-10 w-full border border-input bg-background px-3 text-xs sm:h-12 sm:text-sm`}
            >
            ...
            <div className="flex items-center justify-between">
              ...
              {selectedFeatureOptionIds.length > 0 && (
                <span className="text-xs text-gray-500">
                  {selectedFeatureOptionIds.length} sélectionné{selectedFeatureOptionIds.length > 1 ? "s" : ""}
                </span>
              )}
            </div>
