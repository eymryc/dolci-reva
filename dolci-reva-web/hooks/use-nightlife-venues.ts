...
// ========== Night-clubs publics (front-office, sans auth) ==========

export function usePublicNightClubs(filters?: { search?: string; city?: string }) {
  return useQuery({
    queryKey: ["public", "night-clubs", filters],
    queryFn: async () => {
      const params: Record<string, string> = {};
      if (filters?.search) params.search = filters.search.trim();
      if (filters?.city) params.city = filters.city.trim();
      const response = await api.get("/public/night-clubs", { params });
...
export function usePublicNightClub(id: number) {
  return useQuery({
    queryKey: ["public", "night-clubs", id],
    queryFn: async () => {
      const response = await api.get(`/public/night-clubs/${id}`);
      const data = extractApiData<NightlifeVenue>(response.data);
...
export function useNightClubAvailableAreas(
  nightClubId: number,
  date: string | null,
  time: string | null,
  guests: number
  ...
  return useQuery({
    queryKey: ["public", "night-clubs", nightClubId, "available-areas", date, time, guests],
    queryFn: async () => {
      const response = await api.get(`/public/night-clubs/${nightClubId}/available-areas`, {
        params: { date, time, guests },
      });
      const data = extractApiData<NightClubArea[]>(response.data);
      return data || [];
    },
    enabled: !!nightClubId && !!date && !!time && guests > 0,
  });
}
