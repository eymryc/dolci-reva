...
import { usePublicRestaurant, useBookRestaurant } from '@/presentation/hooks/useRestaurants';
...
export default function RestaurantDetailScreen() {
  ...
  const { data: restaurant, isLoading } = usePublicRestaurant(restaurantId);
  const bookMutation = useBookRestaurant(restaurantId);
  ...
  const handleBooking = async () => {
    ...
    if (!startDate || !endDate) {
      setError("Veuillez sélectionner l'heure d'arrivée et de départ.");
      return;
    }
  ...
  const galleryUrls = restaurant.gallery_images?.length
    ? restaurant.gallery_images.map((g) => g.large_url ?? g.medium_url ?? g.url)
    ...
    <View className="flex-1 bg-white">
      <ScrollView contentContainerClassName="pb-8">
        ...
        <View className="gap-4 p-6">
          <View>
            <Text className="font-rajdhani-bold text-2xl text-theme-secondary">{restaurant.name}</Text>
          ...
          {restaurant.description && (
            <Text className="text-sm leading-5 text-gray-600">{restaurant.description}</Text>
          )}
          ...
          <View className="gap-3 rounded-2xl border border-gray-100 bg-gray-50 p-4">
            <Text className="font-rajdhani-semibold text-base text-theme-secondary">Réserver une table</Text>
            ...
            <View className="flex-row items-center justify-between">
              <Text className="text-sm text-gray-700">Convives</Text>
            ...
            <Input placeholder="Notes (allergies, occasion spéciale...)" value={notes} onChangeText={setNotes} multiline />
