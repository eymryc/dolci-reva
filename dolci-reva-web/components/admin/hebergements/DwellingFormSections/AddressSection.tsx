...
export function AddressSection({ 
  ...
  const [addressSelected, setAddressSelected] = useState(() => Boolean(defaultAddress?.trim()));
  const [showSuggestions, setShowSuggestions] = useState(false);

  const { suggestions, loading: loadingAddress } = useAddressAutocomplete(
    addressSelected ? "" : localAddressValue,
    200
  );
      ...
      <div ref={addressInputRef} className="relative">
        ...
        <div className="relative">
          ...
          <Input
            ...
            onFocus={() => {
              if (!addressSelected && localAddressValue.trim().length >= 2) {
