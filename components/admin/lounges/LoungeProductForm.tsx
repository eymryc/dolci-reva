...
import { NightlifeVenueProductFormData } from "@/types/entities/nightlife-venue.types";
...
const loungeProductSchema = z.object({
...
type LoungeProductFormValues = z.infer<typeof loungeProductSchema>;

interface LoungeProductFormProps {
  onSubmit: (data: NightlifeVenueProductFormData, images?: { mainImage?: File | null; galleryImages?: File[] }) => void;
  onCancel: () => void;
  defaultValues?: NightlifeVenueProductFormData & {
...
export function LoungeProductForm({
  onSubmit,
  onCancel,
  defaultValues,
  isLoading = false,
  loungeProductCategories,
}: LoungeProductFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    setValue,
    watch,
  } = useForm<LoungeProductFormValues>({
    resolver: zodResolver(loungeProductSchema),
  ...
  // Mapping des champs
  const fieldMapping: Record<string, keyof LoungeProductFormValues> = {
  ...
  const { serverErrors, showErrorPanel, clearErrors: clearServerErrors, setShowErrorPanel } = useServerErrors<LoungeProductFormValues>({
    setError,
    fieldMapping,
  });

  const handleFormSubmit = (data: LoungeProductFormValues) => {
    clearServerErrors();
    clearErrors();
    setImageError("");
    ...
    // Trouver le lounge_id à partir de la catégorie sélectionnée
    const selectedCategory = loungeProductCategories.find(cat => cat.id === data.category_id);
    const loungeId = selectedCategory?.venue_id || selectedCategory?.lounge_id || defaultValues?.lounge_id || 0;
    
    // Convertir en NightlifeVenueProductFormData (qui nécessite lounge_id)
    const formData: NightlifeVenueProductFormData = {
      lounge_id: loungeId,
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            {loungeProductCategories.map((category) => (
      ...
      <div className="space-y-2">
        ...
        <Input
          id="name"
          placeholder="Exemple : Mojito, Whisky, Bière locale, Cocktail signature, Soft drink"
          {...register("name")}
      ...
      <div className="space-y-2">
        ...
        <Textarea
          id="description"
          placeholder="Exemple : Cocktail rafraîchissant à base de rhum, menthe fraîche et citron vert"
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          step="0.01"
          min="0.01"
          placeholder="Exemple : 5000 (en FCFA)"
