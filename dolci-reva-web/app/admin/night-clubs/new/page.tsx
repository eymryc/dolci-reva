...
export default function NewNightClubPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async ({
      ...
      appendNightlifeVenueFormData(formData, data, { forceVenueType: "NIGHT_CLUB" });
      appendNightlifeImages(formData, images);

      const response = await api.post("/night-clubs", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      const venue = extractApiData<NightlifeVenue>(response.data);
      if (!venue) throw new Error("Failed to create night-club");
      return venue;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "night-clubs"] });
      toast.success("Night-club créé avec succès !");
      router.push(bo("/night-clubs"));
    },
    onError: (error: unknown) => {
      handleError(error, {
        defaultMessage: "Erreur lors de la création du night-club",
      });
    },
  });
  ...
  const handleCancel = () => router.push(bo("/night-clubs"));
        ...
        <div className="mx-auto w-full max-w-7xl space-y-6">
          ...
          <header className="relative overflow-hidden border border-[#f08400]/25 bg-gradient-to-br from-[#fff4e8] via-[#fffaf5] to-white px-5 py-6 sm:px-7 sm:py-7">
            ...
            <h1 className="relative mt-2 text-[1.85rem] font-semibold leading-[1.1] tracking-tight text-slate-900 sm:text-[2.35rem]">
              Ajouter un night-club
            </h1>
            <p className="relative mt-2.5 max-w-xl text-[15px] leading-relaxed text-slate-500">
              Quelques étapes guidées pour publier votre night-club sur Dolci
              Rêva.
            </p>
          </header>
