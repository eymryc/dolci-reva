...
function toRestaurantCard(
  ...
  return {
    ...
    href: path(`/restaurants/${restaurant.id}`),
    imageUrl: restaurant.main_image_url || restaurant.main_image_thumb_url,
    location: [restaurant.city, restaurant.country].filter(Boolean).join(", "),
    status: restaurant.is_active ? "available" : "inactive",
...
export default function RestaurantsPage() {
  ...
  const filteredCards = useMemo(() => {
    ...
    return restaurants
      ...
      .map((r) => toRestaurantCard(r, bo));
  }, [restaurants, search, bo]);
  ...
  const handleCreateRestaurant = () => {
    router.push(bo("/restaurants/new"));
  };

  const handleEditRestaurant = (restaurant: Restaurant) => {
    router.push(bo(`/restaurants/${restaurant.id}/edit`));
  };
      ...
      <HostShell>
        <div className="mx-auto w-full max-w-6xl">
          <HostPageHeader
            eyebrow="Espace hôte"
            title="Restaurants"
            description="Gérez vos adresses culinaires — ouvrez une fiche pour le détail."
            count={filteredCards.length}
            countLabel={{ singular: "restaurant", plural: "restaurants" }}
            actionLabel="Ajouter un restaurant"
            onAction={handleCreateRestaurant}
            actionDisabled={isLoadingRestaurants || !isOwnerApproved}
            actionTitle={
              !isOwnerApproved
                ? "Votre compte doit être vérifié pour ajouter un restaurant"
                : undefined
            }
            search={search}
          ...
          <div className="mt-8">
            <OwnerEstablishmentGallery
              items={filteredCards}
              isLoading={isLoadingRestaurants}
              emptyTitle="Votre premier restaurant vous attend"
              emptyDescription="Ajoutez une adresse pour commencer à recevoir des clients sur Dolci Rêva."
              onEdit={(id) => router.push(bo(`/restaurants/${id}/edit`))}
              onDelete={(id) => {
                const r = restaurants.find((x) => x.id === id);
                if (r) handleDeleteRestaurant(r);
              }}
            />
          </div>
        </div>
