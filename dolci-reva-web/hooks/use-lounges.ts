...
// ========== Lounges publics (front-office, sans auth) ==========

export function usePublicLounges(filters?: { search?: string; city?: string }) {
  return useQuery({
    queryKey: ["public", "lounges", filters],
    queryFn: async () => {
      const params: Record<string, string> = {};
      if (filters?.search) params.search = filters.search.trim();
      if (filters?.city) params.city = filters.city.trim();
      const response = await api.get("/public/lounges", { params });
...
export function usePublicLounge(id: number) {
  return useQuery({
    queryKey: ["public", "lounges", id],
    queryFn: async () => {
      const response = await api.get(`/public/lounges/${id}`);
      const data = extractApiData<NightlifeVenue>(response.data);
