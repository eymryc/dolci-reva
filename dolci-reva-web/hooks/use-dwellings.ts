...
export function usePublicDwellings(filters?: PublicDwellingsFilters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.structure_type) params.structure_type = filters.structure_type.trim();
      if (filters?.construction_type) params.construction_type = filters.construction_type.trim();
      if (filters?.min_price != null && filters.min_price !== '') params.min_price = String(filters.min_price);
      if (filters?.max_price != null && filters.max_price !== '') params.max_price = String(filters.max_price);
      if (filters?.min_rooms != null && filters.min_rooms !== '') params.min_rooms = String(filters.min_rooms);
      if (filters?.is_available !== undefined && filters.is_available !== '') {
        params.is_available = String(filters.is_available);
      }
...
export function useUpdateDwelling() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // POST + _method=PUT : PHP ne parse pas multipart sur PUT réel
      const response = await api.post<ApiResponse<Dwelling>>(`/dwellings/${id}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
